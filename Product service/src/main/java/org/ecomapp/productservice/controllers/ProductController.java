...
import org.ecomapp.productservice.services.productService.ProductService;
...
public class ProductController {
    ...
    @PostMapping
    public ResponseEntity<ProductResponseDTO> create(@RequestHeader("X-User-Id") Long sellerId, @Valid @RequestBody ProductRequestDTO productRequestDTO) {
        return new ResponseEntity<>(productService.create(sellerId, productRequestDTO), HttpStatus.CREATED);
    }

    @Operation(summary = "Update product details - SELLER only")
    @PatchMapping("/{productId}")
    public ResponseEntity<ProductResponseDTO> update(@PathVariable Long productId, @Valid @RequestBody UpdateProductRequestDTO updateProductRequestDTO) {
        return new ResponseEntity<>(productService.update(productId, updateProductRequestDTO), HttpStatus.OK);
    }

    @Operation(summary = "Update product status - SELLER only")
    @PatchMapping("/{productId}/status")
    public ResponseEntity<Void> changeStatus(@PathVariable Long productId,@Valid @RequestBody ProductStatus status) {
